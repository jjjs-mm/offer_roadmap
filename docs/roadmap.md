...
- [ ] **W2（8/27–9/2）** pytest 已完成；补第二个工具（工作目录内读文件 + 路径防逃逸）并接进 Agent Loop；LeetCode 双指针每天 3 新 + 1 复刷；八股每天 1 题口述并写进 `notes/topics/`
...
工作日每天约 4–5 小时（研二有课按比例压缩，但三条线都要动：算法、项目、八股）：

| 时段 | 内容 | 时长 |
| --- | --- | --- |
| 早 | LeetCode **3 道新题 + 1 道错题复刷** | 2h |
| 中 | 主线项目（今天没切到项目再加题，不要空过） | 1.5–2h |
| 晚 | Agent / Java 八股 1 题：口述 2 分钟并写到 `notes/topics/` | 45min |

周末：一天补项目或写博客，一天休息。强度加在工作日题量和项目推进上，不靠取消休息。
